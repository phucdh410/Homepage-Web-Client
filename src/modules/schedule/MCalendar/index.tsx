  ...
  const renderDateNotes = useCallback(
    (dateNum: number) => {
      ...
      if (date in MOCK) {
        ...
        return (
          ...
          >
            ...
            <div className="hidden md:block max-w-full">
              {MOCK[date].map((el, _i) => (
                <div key={_i} className="flex gap-1">
        ...
        return (
          <div className="px2 py-1 md:px-2 md:py-2 2xl:px-4 2xl:py-3 bg-white min-h-[60px] md:min-h-[80px] 2xl:min-h-[117px] flex flex-col justify-start items-start font-raleway hover:bg-slate-50">
            <div className="px-2">{dateNum + 1}</div>
          </div>
        );
    },
      ...
      <div className=" mx-3 md:mx-5 2xl:mx-10">
        ...
        <div className="grid grid-cols-7 text-center gap-[1px]">
          {weekDays.map((day, i) => (
            <div
              key={i}
              className="text-primary font-semibold font-raleway p-5"
            >
              {day}
            </div>
          ))}
